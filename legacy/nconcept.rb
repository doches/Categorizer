...
class Concept < Array
  ...
  def entails(other,metric)
  	case metric
  		...
  		when :cosine
  		  if self.size == other.size and self.size > 0
    		  return cosine(NVector.to_na(self),NVector.to_na(other))
    		else
    		  STDERR.puts "confusing vectors for ncosine"
    		  STDERR.puts self.size, other.size
    		  return nil
    		end
  	end
  end
